...
class Enemy3:
    ...
    def update(self):
        if self.app.player.scoreStateEnemy == 'false':
            if self.app.stateE3 == 'blinked':
                self.k += 1
                if self.k >= 450:
                    self.app.stateE3 = 'blinked2'
            elif self.app.stateE3 == 'blinked2':
                self.k += 1
                if self.k >= 550:
                    self.app.stateE3 = 'stop'
            elif self.app.stateE3 == 'eaten':
                if self.grid_pos == [16,13]:
                    self.app.stateE3 = 'stop'

        self.target = self.set_target()
        if self.target != self.grid_pos:
            if self.app.stateE3 == 'eaten':
        ...
        if self.app.stateE3 == 'stop':
            ...
            if self.eState == 'right':
                if self.walkcount + 1 >= 32:
                    self.walkcount = 0
                r = self.app.blueR[self.walkcount // 8]
                self.app.screen.blit(r, ((int(self.pix_pos.x) - 10, int(self.pix_pos.y) - 10)))
                self.walkcount += 1
            if self.eState == 'left':
                if self.walkcount + 1 >= 32:
                    self.walkcount = 0
                r = self.app.blueL[self.walkcount // 8]
                self.app.screen.blit(r, ((int(self.pix_pos.x) - 10, int(self.pix_pos.y) - 10)))
                self.walkcount += 1
            if self.eState == 'up':
                if self.walkcount + 1 >= 32:
                    self.walkcount = 0
                r = self.app.blueU[self.walkcount // 8]
                self.app.screen.blit(r, ((int(self.pix_pos.x) - 10, int(self.pix_pos.y) - 10)))
                self.walkcount += 1
            if self.eState == 'down':
                if self.walkcount + 1 >= 32:
                    self.walkcount = 0
                r = self.app.blueD[self.walkcount // 8]
                self.app.screen.blit(r, ((int(self.pix_pos.x) - 10, int(self.pix_pos.y) - 10)))
                self.walkcount += 1
        elif self.app.stateE3 == 'blinked':
            self.speed = 1
            if self.walkcount + 1 >= 32:
                self.walkcount = 0
    ...
    def set_target(self):
        ...
        elif self.app.stateE3 == 'blinked' or self.app.stateE3 == 'blinked2':
