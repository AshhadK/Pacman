...
class Enemy2:
    ...
    def update(self):
        if self.app.player.scoreStateEnemy == 'false':
            if self.app.stateE2 == 'blinked':
                self.k += 1
                if self.k >= 450:
                    self.app.stateE2 = 'blinked2'
            elif self.app.stateE2 == 'blinked2':
                self.k+=1
                if self.k >= 550:
                    self.app.stateE2 = 'stop'
            elif self.app.stateE2 == 'eaten':
                if self.grid_pos == [11,13]:
                    self.app.stateE2 = 'stop'

        self.target = self.set_target()
        if self.target != self.grid_pos:
            if self.app.stateE2 == 'eaten':
        ...
        if self.app.stateE2 == 'stop':
            ...
            if self.eState == 'right':
                if self.walkcount + 1 >= 32:
                    self.walkcount = 0
                r = self.app.pinkR[self.walkcount // 8]
                self.app.screen.blit(r, ((int(self.pix_pos.x) - 10, int(self.pix_pos.y) - 10)))
                self.walkcount += 1
            if self.eState == 'left':
                if self.walkcount + 1 >= 32:
                    self.walkcount = 0
                r = self.app.pinkL[self.walkcount // 8]
                self.app.screen.blit(r, ((int(self.pix_pos.x) - 10, int(self.pix_pos.y) - 10)))
                self.walkcount += 1
            if self.eState == 'up':
                if self.walkcount + 1 >= 32:
                    self.walkcount = 0
                r = self.app.pinkU[self.walkcount // 8]
                self.app.screen.blit(r, ((int(self.pix_pos.x) - 10, int(self.pix_pos.y) - 10)))
                self.walkcount += 1
            if self.eState == 'down':
                if self.walkcount + 1 >= 32:
                    self.walkcount = 0
                r = self.app.pinkD[self.walkcount // 8]
                self.app.screen.blit(r, ((int(self.pix_pos.x) - 10, int(self.pix_pos.y) - 10)))
                self.walkcount += 1
        elif self.app.stateE2 == 'blinked':
            self.speed = 1
            if self.walkcount + 1 >= 32:
                self.walkcount = 0
            ...
            if self.walkcount + 1 >= 32:
                self.walkcount = 0
            r = self.app.blinkedE2[self.walkcount // 8]
            self.app.screen.blit(r, ((int(self.pix_pos.x) - 10, int(self.pix_pos.y) - 10)))
            self.walkcount += 1
    ...
    def set_target(self):
        ...
        elif self.app.stateE2 == 'blinked' or self.app.stateE2 == 'blinked2':
            if self.app.player.grid_pos[0] > COLS // 2 and self.app.player.grid_pos[1] > ROWS // 2:
                return vec(1, 1)
            elif self.app.player.grid_pos[0] > COLS // 2 and self.app.player.grid_pos[1] < ROWS // 2:
                return vec(1, ROWS - 2)
